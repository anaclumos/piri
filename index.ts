...
const copyFilesForLanguage = async ({ content, language }: { content: Content, language: Language }) => {
...
const translateFilesForLanguage = async ({ content, language }: { content: Content, language: Language }) => {
  const targetPath = content.target.replace('LANG', language.code);
